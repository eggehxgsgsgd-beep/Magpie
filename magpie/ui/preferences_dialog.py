...
class PreferencesDialog(QDialog):
    ...
    def _create_categories_tab(self) -> QWidget:
        ...
        self.category_table.setHorizontalHeaderLabels(["快捷键", "类别文件夹名", "显示名称", "颜色"])
        self.category_table.horizontalHeader().setStretchLastSection(True)
        self.category_table.cellDoubleClicked.connect(self._choose_category_color)
    ...
    def _add_category_row(self, category: Category | None = None) -> None:
        ...
        for column, value in enumerate([category.key, category.folder_name, category.display_name, category.color]):
    ...
    def _choose_category_color(self, row: int, column: int) -> None:
        if column != 3:
            return
        item = self.category_table.item(row, column)
        color = QColorDialog.getColor(QColor(item.text() if item else "#4CAF50"), self)
        if color.isValid():
            item = item or QTableWidgetItem()
    ...
    def _collect_categories(self) -> list[Category]:
        ...
        for row in range(self.category_table.rowCount()):
            ...
            color = self._table_text(row, 3) or DEFAULT_PALETTE[row % len(DEFAULT_PALETTE)]
