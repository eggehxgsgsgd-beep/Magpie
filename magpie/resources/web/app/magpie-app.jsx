...
function ConflictDialog({ conflict, onResolve }) {
  ...
  const { cat, target } = conflict;
  const actions = [
    {id:'skip',label:'跳过',bg:'transparent',clr:t.textSec,bdr:`1px solid ${t.border}`},
    {id:'overwrite',label:'覆盖',bg:t.danger,clr:'#fff',bdr:'none'},
  ...
  return (
    <div style={{position:'fixed',inset:0,zIndex:200,display:'flex',alignItems:'center',
      ...
      <div style={{width:440,background:t.dialogBg,borderRadius:12,
        ...
        <div style={{display:'flex',gap:8,justifyContent:'flex-end'}}>
          {actions.map(a => (
            <button key={a.id} onClick={()=>onResolve(a.id, remember)} style={{
...
function MagpieApp({ api, initialPrefs, initialFolder }) {
  const s = useMagpie(api);
  const tweaks = { layout: 'classic', badgeStyle: 'corner' };
  const themeKey = resolveTheme(s.prefs.theme);
  ...
  // Boot: hydrate prefs from backend and open last folder if recorded.
  useEffect(() => {
    if (initialPrefs) {
      s.setPrefs(initialPrefs);
      s.setMode(initialPrefs.default_operation || 'copy');
      s.setBBox(!!initialPrefs.show_bboxes);
    }
    if (initialFolder) {
      s.openFolder(initialFolder);
    }
    // Backend-pushed toasts (e.g. for async ops in future)
    if (api && api.on) {
      const off = api.on('toast', ({msg, color}) => s.showToast(msg, color));
      return () => off && off();
    }
  ...
  useEffect(() => {
    ...
    const handler = (e) => {
      ...
      if (ctrl && (e.key==='o'||e.key==='O')) { e.preventDefault(); s.openFolder(); return; }
      ...
      switch(e.key) {
        ...
        case 'b': case 'B': if(!ctrl) s.setBBox(!s.bbox); break;
        case 'f': case 'F': if(!ctrl) { e.preventDefault(); s.fitToWindow(); } break;
        case '0': if(!ctrl) { e.preventDefault(); s.actualSize(); } break;
        case '+': case '=': if(!ctrl) { e.preventDefault(); s.zoomIn(); } break;
        case '-': case '_': if(!ctrl) { e.preventDefault(); s.zoomOut(); } break;
        default:
          if (ctrl && (e.key==='z'||e.key==='Z')) { e.preventDefault(); e.shiftKey?s.redo():s.undo(); }
            ...
            if (api && api.copyToClipboard && s.img) { e.preventDefault(); api.copyToClipboard(s.img.name); s.showToast('已复制图片名', theme.accent); }
          }
          else if (!ctrl && !e.altKey) {
            const cat = s.cats.find(c => c.key === e.key);
            if (cat) { e.preventDefault(); s.classify(cat.folder_name); }
          }
      }
    };
    window.addEventListener('keydown', handler);
    return () => window.removeEventListener('keydown', handler);
  }, [s.screen, s.settings, s.conflict, s.jump, s.cats, s.bbox,
      s.prev, s.next, s.setPlaying, s.setBBox, s.undo, s.redo,
      s.setJump, s.openFolder, s.classify, s.setSettings, s.img, theme.accent, api,
      s.fitToWindow, s.actualSize, s.zoomIn, s.zoomOut]);

  const applyPrefs = useCallback(async (newPrefs) => {
    if (!api) return;
    const res = await api.savePreferences(newPrefs);
    if (res && res.ok) {
      s.setPrefs(res.preferences || newPrefs);
      s.setMode((res.preferences || newPrefs).default_operation || 'copy');
      s.setBBox(!!(res.preferences || newPrefs).show_bboxes);
      s.setSettings(false);
      s.showToast('设置已保存', theme.accent);
    } else if (res && res.error) {
      s.showToast(res.error, '#EF4444');
    }
  }, [api, s, theme.accent]);
    ...
    <ThemeCtx.Provider value={theme}>
      <div style={{
        ...
        <SettingsDialog open={s.settings} onClose={()=>s.setSettings(false)}
          prefs={s.prefs} onApply={applyPrefs} api={api}/>
